...
class SplitsControllerTest < ActionController::TestCase
  ...
  test "should create split" do
    assert_difference('Split.count') do
      post :create, split: { amount: @split.amount, description: @split.description, status: @split.status }
    end
  ...
  test "should update split" do
    patch :update, id: @split, split: { amount: @split.amount, description: @split.description, status: @split.status }
    assert_redirected_to split_path(assigns(:split))
  end
